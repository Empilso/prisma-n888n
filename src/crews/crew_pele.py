#!/usr/bin/env python3
"""
⚽ CREW PELÉ v2.1 — ORQUESTRADOR DE EMENDAS PARLAMENTARES BA
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
PATHS DE SAÍDA (gerenciados pelo PeleFileManager):
    ...
    → Manifesto: data/saida/pele/.manifest.json  (controle enterprise)

GERENCIAMENTO ENTERPRISE DE ARQUIVOS:
    - Skip inteligente por hash SHA-256 (não apenas por existência)
    - Manifesto persistido com integridade por arquivo
    - --force deleta manifesto do ano antes de reprocessar (rollback limpo)
    - CLI do file manager: python src/shared/pele_file_manager.py saude
...
USO:
    ...
    # Forçar reprocessamento COMPLETO (limpa manifesto do ano)
    python crew_pele.py --pasta ./emendasparlamentares --ano 2024 --origem estadual --force

    # Verificar saúde dos arquivos gerados
    python src/shared/pele_file_manager.py saude

    # Limpar arquivos órfãos (dry-run)
    python src/shared/pele_file_manager.py orfaos

    # Limpar arquivos órfãos (executa de verdade)
    python src/shared/pele_file_manager.py orfaos --exec

    # Purgar todos os arquivos de 2023 (rollback)
    python src/shared/pele_file_manager.py purgar --ano 2023 --exec
"""
...
# Importa o File Manager (graceful — se não encontrar, continua sem ele)
try:
    sys.path.insert(0, str(SHARED_DIR))
    from pele_file_manager import PeleFileManager
    _FM_DISPONIVEL = True
except ImportError:
    _FM_DISPONIVEL = False
...
def print_banner():
    ...
    linhas = [
        ...
        "+ ENTERPRISE FILE MANAGER (hash-based skip)",
    ]
...
def pipeline_origem(
    ...
    for fase in fases_ordem_origem:
        ...
        # ── Skip inteligente via FileManager (hash-based) ──────────────────
        if fase != "D" and not force and not dry_run:
            # Calcula hash da pasta de CSVs de entrada (se aplicável)
            hash_input = None
            if fm and hasattr(fm, 'hash_pasta_csv'):
                pasta_csv = pasta if origem == "estadual" else (pasta_federal or pasta)
                if pasta_csv:
                    hash_input = fm.hash_pasta_csv(Path(pasta_csv))

            if fm and fm.skip_se_existir(fase, origem if fase not in ("A1","A2") else origem, ano, hash_input):
                n = contar_registros(fase, origem, ano)
                resultados[fase] = {"status": "skip", "registros": n}
                continue
            elif not fm and arquivo_existe(fase, origem, ano):
                # Fallback se FileManager não disponível
                n = contar_registros(fase, origem, ano)
                print_status(f"Output já existe ({n} rec). Use --force para re-processar.", "skip")
                resultados[fase] = {"status": "skip", "registros": n}
                continue
        ...
        if fase == "A1":
            ...
            # Pelé-B usa --tipo (estadual|federal), não --origem
            if fase == "B":
                args_extra = ["--tipo", origem, "--ano", ano]
            else:
        ...
        # ── Registra no manifesto após sucesso ────────────────────────────
        if not dry_run and fm and fase != "D":
            p_saida = fase_output(fase, origem, ano)
            if p_saida and p_saida.exists():
                n = contar_registros(fase, origem, ano)
                pasta_csv = pasta if origem == "estadual" else (pasta_federal or pasta)
                hash_input = fm.hash_pasta_csv(Path(pasta_csv)) if pasta_csv and hasattr(fm, 'hash_pasta_csv') else None
                fm.registrar(fase, origem, ano, p_saida, n, hash_input=hash_input)
...
def main():
    ...
    ap.add_argument("--force",        action="store_true",
                    help="Re-processa mesmo que arquivos intermediários já existam (limpa manifesto do ano)")
    ...
    ap.add_argument("--saude",        action="store_true",
                    help="Mostra relatório de saúde dos arquivos e sai")
    args = ap.parse_args()
    ...
    if _FM_DISPONIVEL:
        fm = PeleFileManager()
        print_status(f"File Manager ativo: {fm.manifesto_path}", "info")
    else:
        print_status("PeleFileManager não encontrado — modo legacy (por existência de arquivo)", "warn")
    ...
    if args.saude:
        if fm:
            ...
            print_status("FileManager não disponível para relatório de saúde.", "error")
        return

    # ── --force: purga manifesto do ano antes de reprocessar ─────────────────
    if args.force and fm:
        origens_purgar = ["estadual", "federal"] if args.origem == "ambos" else [args.origem]
        for o in origens_purgar:
            removidos = fm.purgar_ano(args.ano, o, dry_run=False)
            if removidos:
                print_status(f"Force: {removidos} entradas purgadas para {args.ano}/{o}", "warn")
    ...
    relatorio = {
        ...
        "File Manager":        "ATIVO ✅" if fm else "DESATIVADO ⚠️",
    ...
    # Relatório de saúde automático ao final (se FM disponível)
    if fm and not args.dry_run and not algum_falhou:
        fm.relatorio_saude()
