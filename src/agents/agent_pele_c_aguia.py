...
MISSÃO:  Enriquecer os registros Prata (estadual BA) com parlamentar_id
         ...
         o Pelé-D inserir em emendas_estaduais_ba.
...
def main():
    ...
    print(f"\n{C_WHITE}\U0001f3c6 Top 5 deputados por valor empenhado:{C_END}")
    ...
    print(f"\n{C_WHITE}\U0001f4ca Resultado do enriquecimento:{C_END}")
    print(f"   \U0001f517 Cruzados Zidane : {stats['cruzados_zidane']}")
    print(f"   \u26a0\ufe0f  Sem cruzamento : {stats['sem_cruzamento']}")
    print(f"   \U0001f947 Ouro            : {stats['ouro']}")
    print(f"   \U0001f948 Prata           : {stats['prata']}")
    print(f"   \U0001f949 Bronze          : {stats['bronze']}")

    if args.dry_run:
        print(f"\n{C_YELLOW}\u26a0\ufe0f  DRY-RUN: Nenhum arquivo salvo.{C_END}")
        if ouro:
            print_status("Amostra do 1\u00ba registro Ouro (emendas_estaduais_ba):", "info")
